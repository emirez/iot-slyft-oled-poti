...
int TUT4__tutorial4__handle_get___state(coap_rw_buffer_t *scratch,
    ...
    TUT4__state_init(&(resp.data.state_205));


    
    // forward req,resp structs to application handler
    bool handled = TUT4__tutorial4__get___state(&req, &resp);
    ...
    if (handled) {
      ...
      if ( RSPCODE_CLASS(resp_code) == 2 && RSPCODE_DETAIL(resp_code) == 05) {
        ...
        resp_len = resp.data.state_205.TUT4__state_serialize(&(resp.data.state_205), outbuf, sizeof(outbuf));
...
int TUT4__tutorial4__handle_get___config(coap_rw_buffer_t *scratch,
    ...
    TUT4__config_init(&(resp.data.config_205));


    
    // forward req,resp structs to application handler
    bool handled = TUT4__tutorial4__get___config(&req, &resp);
    ...
    if (handled) {
      ...
      if ( RSPCODE_CLASS(resp_code) == 2 && RSPCODE_DETAIL(resp_code) == 05) {
        ...
        resp_len = resp.data.config_205.TUT4__config_serialize(&(resp.data.config_205), outbuf, sizeof(outbuf));
        resp_content_type = COAP_CONTENTTYPE_APPLICATION_OCTECT_STREAM;
...
int TUT4__tutorial4__handle_post___config(coap_rw_buffer_t *scratch,
    ...
    // handle input body
    TUT4__config_init(&(req.data.config));
    int parse_res = req.data.config.TUT4__config_parse(&(req.data.config),inpkt->payload.p, (uint32_t)inpkt->payload.len);
    if (parse_res != 0) {
    ...
    // forward req,resp structs to application handler
    bool handled = TUT4__tutorial4__post___config(&req, &resp);
...
// Protocol-level handler for POST requests to /oled
int TUT4__tutorial4__handle_post___oled(coap_rw_buffer_t *scratch,
    ...
    // handle input body
    TUT4__oled_init(&(req.data.oled));
    int parse_res = req.data.oled.TUT4__oled_parse(&(req.data.oled),inpkt->payload.p, (uint32_t)inpkt->payload.len);
    if (parse_res != 0) {
    ...
    // forward req,resp structs to application handler
    bool handled = TUT4__tutorial4__post___oled(&req, &resp);
